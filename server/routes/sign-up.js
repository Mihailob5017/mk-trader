...
const mongoseSignUpModel = require('../mongodb/user-schema');
...
Router.post('/', async (req, res) => {
  ...
  const doesUserExist = await mongoseSignUpModel.findOne({ username });
  if (doesUserExist)
    res
      .status(400)
  ...
  const isEmailTaken = await mongoseSignUpModel.findOne({ email });
  if (isEmailTaken)
  ...
  const newUser = new mongoseSignUpModel({ ...inputObject });
